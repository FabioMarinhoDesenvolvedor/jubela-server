...
- ~~**Fase 7 (promoções):** rodar a migration em dev e prod~~ → **CONCLUÍDA, em produção** (colunas aplicadas no saneamento de 03/07; confirmado em 13/07: `GET /products` devolve `promoPrice`/`promoEndsAt`). Opcional futuro: tela de **edição** de produto no admin para criar/encerrar promoção depois do cadastro (hoje só há criação); o endpoint `PATCH /products` já valida a promoção.
- **CORS + localhost (decisão do dono):** o commit `614f658` (Lucas) removeu `localhost:5173/3000` da allowlist. Rodar o front local contra a API de prod agora dá erro de CORS. Saída sem mexer em código: definir `CORS_ORIGINS=http://localhost:5173,http://localhost:3000` no Render — **decisão de segurança pendente** (é liberar origem local numa API pública).
- **Fonte da verdade do repositório:** `lucasNasc` commita a partir do fork `fabiojsdev/jubela-server` (merge `286ffc0`). Alinhar com ele que a `master` de `FabioMarinhoDesenvolvedor/jubela-server` é a que o Render deploya, senão a divergência só aparece quando a produção quebra.
...
| 2026-06-23 | **Status deploy Fase 7** | (sem código) | Build + testes verdes. `refactor/backend-kiss` sincronizada no origin (backup, não dispara deploy — Render só faz deploy de `master`). **`master` NÃO atualizado:** subir o código antes da migration quebraria `GET /products` (TypeORM faz SELECT das colunas `promoPrice`/`promoEndsAt` inexistentes em prod). Ordem obrigatória: **1) rodar migration em prod, 2) ff `master`+push.** Migration não roda daqui (sem credenciais do banco de prod). Aguardando o dono. |
| 2026-07-13 | **Hotfix CORS (domínio próprio) + fim da branch de refactor** | `src/main.ts`, `package-lock.json` (cherry-pick), `REFACTOR-BACKEND.md` | **Incidente:** loja no ar em `www.jubelabrasil.com.br` com `GET /products` bloqueado por CORS. **Causa raiz:** allowlist hardcoded em `main.ts` só tinha `jubela-client.vercel.app` (o commit `614f658` do Lucas, "correcao cors", na verdade só *removeu* os localhost) → Nest responde 200 **sem** `Access-Control-Allow-Origin` e o browser bloqueia. **Fix (`0ea99f1`):** apex + `www` na allowlist + suporte à env `CORS_ORIGINS` (lista por vírgula) para não precisar mexer em código na próxima troca de domínio. **Verificado em prod** (`curl -H "Origin: https://www.jubelabrasil.com.br"` → header presente). **Limpeza:** `refactor/backend-kiss` estava totalmente contida na master (único commit exclusivo: `c3e614b`, lock alinhado ao package.json) → cherry-pick p/ master (`016414e`, build de prod reproduzido localmente com `npm ci --omit=dev` + `nest build`, verde) e **branch apagada** (local+remota). **`master` agora é a única branch.** |
